class QueryBuilder {
  ...
  calculateSlidingDates() {
    ...
    switch (this.filters.slidingWindow) {
      case '1d':
        ...
        since = oneDayAgo.toISOString().split('T')[0];
        break;
      case '1w':
        const oneWeekAgo = new Date(today);
        oneWeekAgo.setDate(oneWeekAgo.getDate() - 7);
        since = oneWeekAgo.toISOString().split('T')[0];
        break;
      case '1m':
    ...
    return { sinceDate: since, untilDate: until };
  }
  ...
  build() {
    ...
    // Use sliding window dates if set, otherwise use fixed dates
    const { sinceDate, untilDate } = this.calculateSlidingDates();
