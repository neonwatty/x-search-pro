const StorageManager = {
  ...
  async reorderSearches(orderedIds) {
    const searches = await this.getSavedSearches();
    const searchMap = new Map(searches.map(s => [s.id, s]));
    const reordered = orderedIds.map(id => searchMap.get(id)).filter(Boolean);
    await chrome.storage.sync.set({ savedSearches: reordered });
    return reordered;
  },
  ...
  async createCategory(name, color = '#6b7280') {
    ...
    categories.unshift(trimmedName);
    await chrome.storage.sync.set({ categories });
